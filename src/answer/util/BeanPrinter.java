...
public class BeanPrinter {
	...
	private static Logger log = Logger.getLogger(BeanPrinter.class);
	private static BeanPrinter instance = new BeanPrinter();

	protected static BeanPrinter getInstance() {
		return instance;
	}
	...
	public static String dump(Object o, int maxDepth, int maxArrayElements,
			String[] ignoreList) {
		DumpContext ctx = BeanPrinter.getInstance().new DumpContext();
	...
	protected static String dump(Object o, DumpContext ctx) {
		...
		if (oClass.isArray()) {
			...
			if (rowCount < Array.getLength(o)) {
				buffer.append(tabs.toString());
				buffer.append(Array.getLength(o) - rowCount
						+ " more array elements...");
				buffer.append("\n");
			}
			buffer.append(tabs.toString().substring(1));
			...
			while (oClass != null && oClass != Object.class) {
				...
				if (ctx.ignoreList.get(oClass.getSimpleName()) == null) {
					if (oClass != o.getClass()) {
						buffer.append(tabs.toString().substring(1));
						buffer.append("  Inherited from superclass "
								+ oSimpleName + ":\n");
					}
					for (int i = 0; i < fields.length; i++) {
						String fSimpleName = getSimpleNameWithoutArrayQualifier(fields[i]
								.getType());
